...
const FeedScreen = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axios.get("http://192.168.1.15:8000/api/get_userrequests", {
                headers: {
    ...
    const fetchDonation = async () => {
        try {
            const authToken = await AsyncStorage.getItem("authToken");
            const response = await Axios.get("http://192.168.1.15:8000/api/get_lastdonation", {
                headers: {
                    Authorization: `Bearer ${authToken}`,
                },
            });
                ...
                <View style={styles.requestsContainer}>
                    ...
                    >
                        {requestsData.map((bloodRequest, index) => (
                            <RequestCard
                                key={index}
                                imageSource={{ uri: `http://192.168.1.15:8000/storage/${bloodRequest.hospital_info.logo_url}` }}
